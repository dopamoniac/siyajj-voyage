    ...
    <div className="container mx-auto px-4 md:px-8 py-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="glass-card p-6 rounded-xl border border-white/5 hover:border-siyajj-luxury-gold/30 transition-all">
            <h3 className="text-xl font-serif text-siyajj-ivory mb-2">{item.title}</h3>
            <p className="text-sm text-siyajj-muted-text mb-4">{item.desc}</p>
            <button className="text-[10px] uppercase tracking-widest text-siyajj-luxury-gold">En savoir plus</button>
          </div>
        ))}
      </div>
    </div>
